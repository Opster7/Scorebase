...
def euler_sample(score_model, marginal_prob_std, diffusion_coeff, batch_size=64, num_steps = num_steps, device='cuda', eps=1e-3):
    # Step1 定义初始时间1和先验分布的随机样本（t从1到0反时间采样）
    t = torch.ones(batch_size, device=device)  # t=1
    init_x = torch.randn(batch_size, 1, 28, 28, device=device) * marginal_prob_std(t)[:, None, None,
